...
class Standard extends Base implements Iface
{
	...
	{
		return (string) $this->get( 'order.service.siteid', '' );
	}
	 ...
	 * @return static Order base service item for chaining method calls
	 */
	public function setServiceId( string $servid ) : static
	{
	 ...
	 * @return static Order service item for chaining method calls
	 */
	public function fromArray( array &$list, bool $private = false ) : static
	{
		...
		foreach( $list as $key => $value )
		{
			switch( $key )
			{
				case 'order.service.siteid': !$private ?: $item->setSiteId( (string) $value ); break;
				case 'order.service.parentid': !$private ?: $item->setParentId( $value ? (string) $value : null ); break;
				case 'order.service.serviceid': !$private ?: $item->setServiceId( (string) $value ); break;
				case 'order.service.type': $item->setType( (string) $value ); break;
				case 'order.service.code': $item->setCode( (string) $value ); break;
				case 'order.service.name': $item->setName( (string) $value ); break;
				case 'order.service.currencyid': $price = $price->setCurrencyId( (string) $value ); break;
				case 'order.service.price': $price = $price->setValue( (string) $value ); break;
				case 'order.service.costs': $price = $price->setCosts( (string) $value ); break;
				case 'order.service.rebate': $price = $price->setRebate( (string) $value ); break;
				case 'order.service.taxrates': $price = $price->setTaxRates( (array) $value ); break;
				case 'order.service.taxvalue': $price = $price->setTaxValue( (string) $value ); break;
				case 'order.service.taxflag': $price = $price->setTaxFlag( (bool) $value ); break;
				case 'order.service.position': $item->setPosition( $value !== null ? (int) $value : null ); break;
				case 'order.service.mediaurl': $item->setMediaUrl( (string) $value ); break;
				default: continue 2;
			}
	 ...
	 * @return static Order base service item for chaining method calls
	 */
	public function copyFrom( \Aimeos\MShop\Service\Item\Iface $service ) : static
	{
